...
class Model(nn.Module):
    ...
    # def __init__(self, layers, d_hid, frames, num_coeff_Time_kept, model_downsample_rate):
        ...
        self.regress_head = nn.Linear(args.d_hid, 3, bias=False)
    ...
    def forward(self, x):
        ...
        x_t = dct.dct(x.permute(0, 2, 3, 1))[:, :, :, :self.num_Time_kept]
        x_t = x_t.permute(0, 3, 1, 2).contiguous()     

        x = x[:, ::self.model_ds_rate, :, :] 
...
class FTSC_ATTENTION(nn.Module):
    def __init__(self, d_time, d_joint, d_coor, num_T_k, m_ds_r, head=8):
        ...
        self.emb =  nn.Embedding(5, d_coor//head//2, padding_idx=0)
    ...
    def forward(self, input):
        ...
        x_s = att_s @ v_s + sep2_s + 0.0001*self.drop(sep_s)[0]  # b*h,s,c//h
        x_t = att_t @ v_t + sep2_t  # b*h,t,c//h
...
class FTSC_BLOCK(nn.Module):
    def __init__(self, d_time, d_joint, d_coor, num_T_k, m_ds_r):
        ...
        self.cross_att = FTSC_ATTENTION(d_time, d_joint, d_coor, num_T_k, m_ds_r)
...
if __name__ == "__main__":
    ...
    net = Model()
    # net = Model(layers=6, d_hid=256, frames=27, num_coeff_Time_kept=4, model_downsample_rate=2)
    output = net(inputs)
    print(output.size())
